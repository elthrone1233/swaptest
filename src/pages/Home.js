...
function App() {
  ...
  const summarizeRisk = (data) => {
    ...
    if (data.cannot_sell_all !== "0")
      issues.push("you may not be able to sell all your tokens");
    if (data.can_take_back_ownership !== "0")
      issues.push("ownership can be taken back");
    if (data.slippage_modifiable !== "0")
      issues.push("slippage can be changed");

    if (issues.length === 0) {
      return {
        text: "✅ Token looks safe. DYOR always.",
        color: "text-green-400",
      };
    } else if (issues.length === 1) {
      return {
        text: `⚠️ 1 minor risk: ${issues[0]}`,
  ...
  return (
    <div className="bg-white-950 text-black p-6 shadow p-3 mb-5 bg-body rounded-20 main_css">
      {/* Header */}
    ...
    {/* Token Analysis */}
<section ref={analysisRef} className="mt-6 bg-purple-900 p-6 rounded-xl">
  
  {tokenData ? (
    <>
      {/* Dex Chart + Swap Widget Side-by-Side */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
        {/* Dexscreener Chart (2/3 width) */}
        <div className="lg:col-span-2 relative overflow-hidden" style={{ height: "460px" }}>
          ...
          <iframe
            ...
            style={{
              border: "none",
              marginBottom: "-60px", // hide Dexscreener logo
            }}
            allowFullScreen
          ></iframe>
        </div>

        {/* Swap Widget (1/3 width) */}
        <div className="bg-black p-4 rounded-xl overflow-hidden relative" style={{ height: "460px" }}>
    ...
    <p className="text-gray-400 text-center">
      🔍 Search or select a token to see live data and risks.
    </p>
  )}
</section>
