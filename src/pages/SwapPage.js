        ...
        <div className="mb-6">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-white ${slippage === '0.5' ? 'bg-purple-700' : 'bg-purple-700 hover:bg-purple-600'}`}
            >
              0.5%
            </button>
            <button
              onClick={() => setSlippage('1')}
              className={`px-4 py-2 rounded-xl text-white ${slippage === '1' ? 'bg-purple-700' : 'bg-purple-700 hover:bg-purple-600'}`}
            >
              1%
            </button>
            <input
        ...
        <div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-white ${tradeOption === 'default' ? 'bg-purple-700' : 'bg-purple-700 hover:bg-purple-600'}`}
            >
              Default
            </button>
      ...
      <div className="bg-[#1f1f1f] rounded-2xl w-full max-w-md m-4 p-6 border border-gray-700 animate-fade-in-up">
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto mt-4 custom-scrollbar"> {/* Added custom-scrollbar class */}
          {filteredTokens.map(token => (
            <button key={token.symbol} onClick={() => { onSelectToken(token); onClose(); }} className="w-full flex items-center p-3 hover:bg-black rounded-xl text-left transition-colors">
    ...
    <div className="bg-[#191919] p-4 rounded-2xl border border-gray-800">
      ...
      <div className="flex justify-between items-center">
        <input
          ...
          disabled={!isInput}
        />
        <button onClick={onSelectTokenClick} className="flex items-center bg-[#2b2b2b] hover:bg-purple-700 text-white font-semibold py-2 px-3 rounded-2xl transition-colors">
          {selectedToken ? (
    ...
    <>
      <div className="bg-[#121212] p-4 rounded-2xl border border-gray-800 w-full max-w-md mx-auto">
        ...
        <div className="relative space-y-2">
          ...
          <button onClick={handleSwapTokens} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 bg-[#191919] border-4 border-[#121212] rounded-full p-2 text-white hover:bg-black transition-transform duration-200 hover:rotate-180">
          ...
          {walletAddress ? (
            <>
              <button
                ...
                className="w-full bg-purple-600 text-white font-bold py-3 rounded-2xl hover:bg-purple-700 transition-colors text-lg"
              ...
              <button
                onClick={handleSwap}
                className="w-full bg-purple text-white font-bold py-4 rounded-2xl hover:bg-opacity-90 transition-colors text-lg"
  ...
  return (
    <header className="grid grid-cols-3 items-center p-4 border-b border-gray-800">
        ...
        <div className="flex items-center justify-end gap-3">
            <button className="flex items-center gap-2 bg-[#191919] hover:bg-purple-700 text-white font-semibold py-2 px-3 rounded-xl transition-colors">
                <img src={mockTokens[0].logo} alt="Ethereum" className="w-5 h-5 rounded-full" />
                <span>Ethereum</span>
                <ChevronDownIcon />
            </button>
            <button onClick={connectWallet} className="bg-purple text-white font-bold py-2 px-4 rounded-xl hover:bg-opacity-90 transition-colors">
    ...
    return (
        <aside className="w-64 p-4 border-r border-gray-800">
            <nav className="space-y-2">
                <button onClick={() => setActiveSubView('tokens')} className={`w-full text-left px-4 py-2 rounded-lg text-md font-semibold ${activeSubView === 'tokens' ? 'bg-black text-white' : 'text-gray-400 hover:bg-black hover:text-white'}`}>Tokens</button>
                <button onClick={() => setActiveSubView('pools')} className={`w-full text-left px-4 py-2 rounded-lg text-md font-semibold ${activeSubView === 'pools' ? 'bg-black text-white' : 'text-gray-400 hover:bg-black hover:text-white'}`}>Pools</button>
                <button onClick={() => setActiveSubView('transactions')} className={`w-full text-left px-4 py-2 rounded-lg text-md font-semibold ${activeSubView === 'transactions' ? 'bg-black text-white' : 'text-gray-400 hover:bg-black hover:text-white'}`}>Transactions</button>
            </nav>
        </aside>
    );
}
...
function Message({ message, type, onDismiss }) {
    ...
    const styles = { success: "bg-green-500", error: "bg-red-500", info: "bg-purple-500" };
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div className="bg-black min-h-screen font-sans text-white relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-purple-900 rounded-full mix-blend-screen filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
